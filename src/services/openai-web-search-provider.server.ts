...
export const openAiWebSearchProvider: BusinessResearchProvider = {
  ...
  async research(query) {
    ...
    if (!apiKey)
      throw new Error(
        "OpenAI API key is missing. Configure OPENAI_API_KEY on the server and restart it.",
      );
    ...
    try {
      ...
      const response = await client.responses.create({
        model:
          process.env["OPENAI_RESEARCH_MODEL"] || process.env["OPENAI_MODEL"] || "gpt-4.1-mini",
        store: false,
        ...
        text: {
          format: {
            type: "json_schema",
            name: "grounded_business_research",
            strict: true,
      ...
      if (!response.output_text)
        throw new Error("OpenAI web search returned no structured findings.");

      const parsed = findingsSchema.safeParse(JSON.parse(response.output_text));
      if (!parsed.success)
        throw new Error("OpenAI web search returned an invalid structured response.");
      const searchedSources = sourcesFromResponse(response);
      if (searchedSources.size === 0)
        throw new Error("OpenAI web search returned no source URLs to verify the findings.");
      ...
      return parsed.data.findings
        .flatMap((finding) => {
          ...
          const fields = {
            businessName: asNonEmpty(finding.fields.businessName),
            industry: asNonEmpty(finding.fields.industry),
            phone: asNonEmpty(finding.fields.phone),
            email: asNonEmpty(finding.fields.email),
            address: asNonEmpty(finding.fields.address),
            serviceAreas: finding.fields.serviceAreas.filter(Boolean),
            services: finding.fields.services.filter(Boolean),
            businessDescription: asNonEmpty(finding.fields.businessDescription),
            websiteUrl: asNonEmpty(finding.fields.websiteUrl),
            socialProfiles: finding.fields.socialProfiles.filter(Boolean),
            registrationDate: asNonEmpty(finding.fields.registrationDate),
      ...
      throw new Error(
        "OpenAI web-search API failure. Check the server network connection, model configuration, and API access.",
      );
    }
  },
};
