...
export const claimPrivateProspectDemo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { requireAuthenticatedCustomer } = await import("@/lib/supabase/server");
    const { client } = await requireAuthenticatedCustomer(data.accessToken);
    const { data: rows, error } = await client.rpc("claim_private_prospect_demo", {
      p_preview_token: data.previewToken,
      p_claim_token: data.claimToken,
      p_presentation_overrides: data.presentationOverrides ?? null,
    });
    ...
    if (error) {
      ...
      if (error.code === "22023")
        return authorizationFailure(400, "This private preview cannot be converted into a draft.");
      console.error("Transactional private-demo claim failed", {
        code: error.code,
        message: error.message,
      });
      throw new Error("Unable to claim this private preview right now.");
    }

    const claimed = Array.isArray(rows) ? rows[0] : rows;
    if (!claimed?.website_id) throw new Error("Unable to finish claiming this private preview.");
    return {
      websiteId: claimed.website_id as string,
