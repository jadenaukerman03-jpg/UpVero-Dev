-- The quota RPC is server-only. Customer authorization is completed in the
-- application before this privileged operation is invoked.
drop function public.consume_provider_operation_quota(text);

create function public.consume_provider_operation_quota(
  p_owner_id uuid,
  p_operation text
)
returns boolean
...
begin
  if p_owner_id is null then
    raise exception 'A quota owner is required.' using errcode = '22004';
  end if;
